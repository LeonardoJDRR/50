...
print(f"La suma de los primeros {n} números naturales es: {resultado}")
 ...
 .section .data
prompt:         .asciz "Ingrese un número N: "     // Mensaje para solicitar un número
buffer:         .space 16                          // Espacio para almacenar la entrada del usuario
resultado_msg:  .asciz "La suma es: "              // Mensaje para mostrar el resultado
...
_start:
    ...
    ldr x1, =prompt                                 // x1 = dirección de la cadena "Ingrese un número N:"
    ...
    // x0 ahora contiene el valor de N
    // Calcular la suma de los primeros N números naturales usando la fórmula
    mov x2, x0                                      // Guardar el valor de N en x2
    add x2, x2, #1                                  // x2 = N + 1
    mul x0, x0, x2                                  // x0 = N * (N + 1)
    lsr x0, x0, 1                                   // x0 = (N * (N + 1)) / 2 (división por 2)

    // Imprimir el mensaje de resultado
    ldr x1, =resultado_msg                          // Dirección del mensaje "La suma es: "
    mov x2, #12                                     // Longitud del mensaje
    mov x0, #1                                      // Descriptor de archivo para stdout
    mov x8, #64                                     // Código de sistema para write
    svc 0                                           // Llamada al sistema

    // Convertir el resultado en x0 a cadena y almacenarlo en el buffer
    mov x1, x0                                      // Pasar el resultado a x1
    ldr x0, =buffer                                 // Pasar la dirección del buffer
...
ASCII:https://asciinema.org/a/RP3VRXNf8c6GnXs3xXZmgjDRG
